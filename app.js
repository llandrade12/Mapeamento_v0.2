...
function makeRecord(record) { return { id: record.id || crypto.randomUUID(), type: record.type, name: record.name || "Sem nome", department: record.department || "", ownerId: record.ownerId || "", ownerIds: Array.isArray(record.ownerIds) ? record.ownerIds : [], details: record.details || {}, attachments: Array.isArray(record.attachments) ? record.attachments : [], tags: Array.isArray(record.tags) ? record.tags : [], notes: record.notes || "", createdAt: record.createdAt || new Date().toISOString() }; }
...
function cardTemplate(record) { const owner = getOwner(record); const owners = record.type === "office" ? getOwners(record) : []; const tags = (record.tags || []).map((tag) => '<span class="tag">' + escapeHtml(tag) + '</span>').join(""); const ownerHtml = record.type === "office" && owners.length ? '<p><strong>Funcionários:</strong> ' + owners.map((p) => escapeHtml(p.name)).join(", ") + '</p>' : (owner ? '<p><strong>Funcionário:</strong> ' + escapeHtml(owner.name) + '</p>' : ''); return '<article class="card"><div class="card-header"><div><h4>' + escapeHtml(record.name) + '</h4><p>' + escapeHtml(typeLabels[record.type]) + (record.department ? ' · ' + escapeHtml(record.department) : '') + '</p></div><span class="badge">' + escapeHtml(typeLabels[record.type]) + '</span></div><div class="card-meta">' + ownerHtml + cardDetails(record) + attachmentLinks(record) + (record.notes ? '<p>' + escapeHtml(record.notes) + '</p>' : '') + '</div>' + (tags ? '<div class="tag-list">' + tags + '</div>' : '') + '<div class="card-actions"><button class="text-btn" data-edit="' + record.id + '">Editar</button></div></article>'; }
...
function getOwners(record) { const ids = record.ownerIds?.length ? record.ownerIds : (record.ownerId ? [record.ownerId] : []); return ids.map((id) => records.find((item) => item.id === id && item.type === "person")).filter(Boolean); }
function linkedRecords(personId) { return records.filter((record) => record.ownerId === personId || (Array.isArray(record.ownerIds) && record.ownerIds.includes(personId))); }
function updateOwnerOptions() { const people = records.filter((record) => record.type === "person").sort((a, b) => a.name.localeCompare(b.name, "pt-BR")); const current = fields.owner.value; fields.owner.innerHTML = '<option value="">Sem funcionário vinculado</option>' + people.map((person) => '<option value="' + person.id + '">' + escapeHtml(person.name) + (person.details?.username ? ' (' + escapeHtml(person.details.username) + ')' : '') + '</option>').join(""); fields.owner.value = people.some((person) => person.id === current) ? current : ""; const role = dynamicFields.querySelector('[data-detail="role"]')?.value || ""; const type = fields.type.value; const hideOwner = unassignedTypes.includes(type) || (type === "phone" && role === "Principal") || type === "office"; fields.owner.disabled = hideOwner; ownerWrap.classList.toggle("hidden", hideOwner); if (type === "office") { renderMultiOwnerSelect(getSelectedOwnerIds()); } else { document.querySelector("#multiOwnerWrap")?.remove(); } }
function getSelectedOwnerIds() { return [...(document.querySelectorAll("#multiOwnerSelect option:checked") || [])].map((opt) => opt.value); }
function renderMultiOwnerSelect(selectedIds = []) { document.querySelector("#multiOwnerWrap")?.remove(); const people = records.filter((record) => record.type === "person").sort((a, b) => a.name.localeCompare(b.name, "pt-BR")); const wrap = document.createElement("label"); wrap.id = "multiOwnerWrap"; wrap.className = "wide"; wrap.innerHTML = '<span>Funcionários vinculados</span><select id="multiOwnerSelect" multiple size="' + Math.min(people.length + 1, 6) + '"><option value="">Nenhum</option>' + people.map((person) => '<option value="' + person.id + '"' + (selectedIds.includes(person.id) ? ' selected' : '') + '>' + escapeHtml(person.name) + (person.details?.username ? ' (' + escapeHtml(person.details.username) + ')' : '') + '</option>').join("") + '</select><p class="field-hint">Segure Ctrl (ou Cmd) para selecionar mais de um funcionário.</p>'; ownerWrap.insertAdjacentElement("afterend", wrap); }
...
function openModal(record = {}) { const isEditing = Boolean(record.id); const type = record.type || "person"; document.querySelector("#modalTitle").textContent = isEditing ? "Editar registro" : "Novo registro"; document.querySelector("#deleteRecord").classList.toggle("hidden", !isEditing); fields.id.value = record.id || ""; fields.type.value = type; fields.name.value = record.name || ""; fields.department.value = record.department || ""; fields.tags.value = (record.tags || []).join(", "); fields.notes.value = record.notes || ""; prepareForm(type, record.details || {}); if (type === "office") { renderMultiOwnerSelect(record.ownerIds || (record.ownerId ? [record.ownerId] : [])); } else { fields.owner.value = record.ownerId || ""; } renderAttachmentsPanel(record); renderPersonLinks(record); modal.showModal(); fields.name.focus(); }
async function saveRecord() { const id = fields.id.value || crypto.randomUUID(); const type = fields.type.value; const details = collectDetails(); const attachments = [...currentAttachments, ...(await readAttachmentFiles())]; const ownerIds = type === "office" ? [...(document.querySelectorAll("#multiOwnerSelect option:checked") || [])].map((opt) => opt.value).filter(Boolean) : []; const ownerId = type === "office" ? (ownerIds[0] || "") : getOwnerIdForSave(type, details); const next = makeRecord({ id, type, name: fields.name.value.trim(), department: fields.department.value.trim(), ownerId, ownerIds, details, attachments, tags: fields.tags.value.split(",").map((tag) => tag.trim()).filter(Boolean), notes: fields.notes.value.trim(), createdAt: records.find((record) => record.id === id)?.createdAt }); records = records.some((record) => record.id === id) ? records.map((record) => record.id === id ? next : record) : [next, ...records]; syncNotebookUsername(next); persist(); modal.close(); render(); }
...
function exportCsv() { const header = ["tipo", "nome", "setor", "funcionario_vinculado", "campo_principal", "detalhes", "anexos", "tags", "observacoes"]; const rows = records.map((record) => { const ownerName = record.type === "office" ? getOwners(record).map((p) => p.name).join("; ") : (getOwner(record)?.name || ""); return [typeLabels[record.type], record.name, record.department, ownerName, primaryValue(record), detailsSummary(record), (record.attachments || []).map((file) => file.name).join("; "), (record.tags || []).join("; "), record.notes]; }); const csv = [header, ...rows].map((row) => row.map(csvCell).join(",")).join("\n"); downloadFile('mapa-escritorio-' + dateStamp() + '.csv', csv, "text/csv;charset=utf-8"); }
...
function excelSheet(name, type) { const schema = schemas[type] || []; const headers = ["Nome", "Setor", "Funcionário(s) vinculado(s)", ...schema.map((item) => item.label), "Anexos", "Tags", "Observações"]; const rows = records.filter((record) => record.type === type).map((record) => { const ownerName = type === "office" ? getOwners(record).map((p) => p.name).join("; ") : (getOwner(record)?.name || ""); return [record.name, record.department, ownerName, ...schema.map((item) => record.details?.[item.key] || ""), (record.attachments || []).map((file) => file.name).join("; "), (record.tags || []).join("; "), record.notes]; }); return { name, rows: [headers, ...rows] }; }
...
function reportSection(title, type) { const list = records.filter((record) => record.type === type); if (!list.length) return '<h2>' + escapeHtml(title) + '</h2><p>Nenhum registro.</p>'; return '<h2>' + escapeHtml(title) + '</h2><table><thead><tr><th>Nome</th><th>Setor</th><th>Funcionário(s) vinculado(s)</th><th>Detalhes</th><th>Anexos</th><th>Observações</th></tr></thead><tbody>' + list.map((record) => { const ownerNames = record.type === "office" ? getOwners(record).map((p) => p.name).join(", ") : (getOwner(record)?.name || ""); return '<tr><td>' + escapeHtml(record.name) + '</td><td>' + escapeHtml(record.department) + '</td><td>' + escapeHtml(ownerNames) + '</td><td>' + escapeHtml(detailsSummary(record)) + '</td><td>' + escapeHtml((record.attachments || []).map((file) => file.name).join("; ")) + '</td><td>' + escapeHtml(record.notes) + '</td></tr>'; }).join("") + '</tbody></table>'; }
